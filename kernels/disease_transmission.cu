...
__global__ void diseaseTransmissionKernel(AgentData d_agents,
    ...
    if (idx >= num_infected_agents) {
        return;
    }

    int agent_idx = d_infectious_agent_indices[idx];

    curandState rngState = d_agents.rngStates[agent_idx];
    float x1 = d_agents.x[agent_idx];
    float y1 = d_agents.y[agent_idx];
    int cellId = d_agents.cellId[agent_idx];
    ...
    d_agents.rngStates[agent_idx] = rngState;
}